...
export default {
  ...
  methods: {
    async getHtmlList() {
      let that = this;
      try {
        let result = await that.api.getTps({}, that);
    ...
    async submit() {
      ...
      try {
        let result = await that.crud.add(that.tpModel);
        if (result.code === 200) {
          that.$hint({ msg: "添加成功" });
          that.tpModelReset();
        }
    ...
    async tpUpdate() {
      let that = this;
      try {
        let result = await that.crud.update(that.tpModel);
    ...
    async tpDelete(id) {
      ...
      that.bus.$on("toastConfirm", async function () {
        try {
          let result = await that.crud.delete({ id });
          if (result.code === 200) {
            that.tpQueryAll();
